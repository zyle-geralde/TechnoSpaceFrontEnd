    ...
    <div className="sticky top-0 z-50 backdrop-blur-sm border-3 border-b-primary-color p-2">
        <div className="text-xs flex flex-row self-center sm:px-10 md:px-20 lg:px-30 xl:px-40 ">
            ...
            <div className="ml-auto gap-4 hidden sm:flex">
                <LandingPageButton styleClass={"border-2 px-3 text-xs hover:bg-slate-200 nav-text-hover transition-all"} onClick={()=>{navigate('/Signup')}} label={"Join"} />
                <LandingPageButton styleClass="px-3 bg-primary-color text-white rounded-sm text-xs hover:bg-red-600" onClick={()=>{navigate('/Login')}} label="Login" />
            </div>
        </div>
    </div>
    )
